...
PDF、扫描卷必须先生成可编辑重排正文。逐页图片或页面截图只能放在正文之后作为校验附录，不能用空白 PDF 嵌入框冒充保真，也不能把整页图片作为正文主体：

```html
<main class="exam-layout">
  <!-- 这里必须是 OCR/结构化后的题目重排正文 -->
</main>

<section class="source-pages">
  <h2>原卷校验附录</h2>
  <p class="source-note">以下图片仅用于核对原题、图形和公式，不替代前文重排正文。</p>
...
如果当前工具链只能拿到逐页图片，不能拿到 OCR 文本或题目结构，不要生成贴图式 HTML。应说明“当前缺少 OCR 文本/题目结构化结果”，等待后端提供可重排内容。
