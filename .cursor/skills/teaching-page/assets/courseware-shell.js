...
(function () {
  ...
  function buildSrcdoc(page, sharedHead, state, index, total) {
    var meta =
      'window.__CW_MODE__="main";window.__CW_PAGE__={id:' +
      (page.id || 0) +
      ',index:' +
      (index || 0) +
      ',total:' +
    ...
    // 版式框自动填充：页码 [data-fx-pageno]、页名 [data-fx-pagename]、进度点 [data-fx-progress]
    var frameFill =
  ...
  CoursewareShell.prototype.show = function (index, direction) {
    ...
    this.iframe.srcdoc = buildSrcdoc(page, this.sharedHead, saved, index, this.pages.length);
