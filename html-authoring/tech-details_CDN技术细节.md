...
数学 3D 几何/立体图形/体积表面积/空间关系演示：优先 Three.js，并按 `math-design/grid-templates.md` 创建 3D 场景内的标准地面网格对象；页面 `#stage` / `canvas` 容器背景保持纯色或色板背景，禁止用 CSS `linear-gradient`/`background-image` 伪装地面网格。地面网格必须肉眼可见且模板级可测试：`window.__math3d.ground/grid` 存在，`grid.material.opacity >= 0.55`，`ground.material.opacity <= 0.25`，`grid.position.y > ground.position.y`，`controls.enablePan === false`，`controls.maxPolarAngle < Math.PI/2`。renderer 初始化后必须在第一次渲染前同步 `camera.aspect = container.clientWidth / container.clientHeight` 并 `camera.updateProjectionMatrix()`；`test_html` 需断言相机宽高比与 renderer/container 宽高比误差 ≤0.02，避免正方体/球体被宽屏舞台拉伸。网格色与地面/背景要有明显明度差，地面材质透明且不遮挡模型，相机/OrbitControls 不允许转到地面以下。若使用 p5 WEBGL 或 SVG 伪 3D，也必须在渲染场景内部表达地面/底面参照，不能使用页面平面底纹。
